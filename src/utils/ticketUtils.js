...
async function closeTicket(channel, closedBy, reason) {
  ...
  try {
    ...
    const logsUrl = await postToBin(content, `Logi ticketów ${channel.name}`);
    ...
    const embed = new MessageEmbed().setAuthor({ name: "Ticket Zamknięty" }).setColor(EMBED_COLORS.TICKET_CLOSE);
    if (reason) embed.addField("Reason", reason, false);
    embed
      .setDescription(`**Title:** ${ticketDetails.title}`)
      .addField("Otworzony przez:", ticketDetails.user ? ticketDetails.user.tag : "Użytkownik wyszedł", true)
      .addField("Zamknięty przez:", closedBy ? closedBy.tag : "Użytkownik wyszedł", true);
    ...
    if (ticketDetails.user) {
      const dmEmbed = embed
        .setDescription(`**Na serwerze:** ${channel.guild.name}`)
